...
这篇文章是强行插播过来的，本来是在做 GraphQL 的调研，但是发现其实 REST 这边的很多进展根本就没有说啊，那后面比较的时候很多东西就有点说不清楚了呢。所以就介绍下目前 REST API 开发过程中引入的一些不错的实践。
...
自我一篇很古老的[文章]({% post_url 2016-04-20-some-tips-for-ddd %})里就提到了，`GET` 请求获取的是「读模型」，应该按照调用方的需求合理的展示相关的数据。不过当时将的不够深入，实际上 `GET` 所返回的数据不是孤立的，而是相互关联的。例如有如下两个 API：
...
blog 下的 author 和 users 下的每个 user 的结构如果没什么特殊原因应该是一致的，否则前端同学肯定一脸懵逼。这就是所谓的「结构标准化」了。**就是 REST `GET` 的返回结果应该按照 API 服务所提供的资源之间的关系有统一的结构。**

而「写」数据则不然，它通常是让前端去提交某种结果，如果提交的结构和返回的结构完全一致的话会有两个问题：

1. 可能会非常冗余，让前端觉得比较繁(sha)琐(bi)
2. 容忍度不太高，使用者稍有不慎就会 400 警告
...
其中这个 `price` 部分有点点小冗余了，如果改成以下方式可能会更友好：
...
当然，容忍度高这部分我觉得就有点额外发挥的意思了，甚至会画蛇添足。将 API 调用 SDK 化可以极大程度上减少这种额外工作的必要。
...
OpenAPI 不是什么大厂，没有很强的运营推广能力，很多东西的贡献也全凭兴趣，持久性和可靠性令人怀疑，就拿上面的那些工具链来说吧，可以说是漏洞百出...

redoc 并不支持完整的 openapi 语法，很多时候校验全凭效果，比如 `allOf` `oneOf` 这些继承语法。正如上文所说，「**如果展示的对就是正确的了**」。类似的问题在 `openapi-generator-cli` 也会出现，问题也是千奇百怪。整体来说就是，我们只能使用一个 openapi 语法的子集，并且我觉得如果 openapi 没有能力让下面的工具链与自己的标准协同更新的化，openapi 也就慢慢成了一纸空文。最多就是做了一个 openapi 的方言罢了。而 `openapi2schema` 就更没谱了，就是个完完全全的个人作品，已经一年多没有更新了，后面还能不能用都不知道。
